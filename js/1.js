	...
	***************************/
	...
	$.fn.jqTransSelect = function(){
		return this.each(function(index){
			...
			(function()
			{
				var sel = $select[0], items = $ul.find('a');
				items.each(function(i)
				{
					var item = $(this).click(function(e)
					{
						sel.selectedIndex = item.attr('index');
						$select.change();
						e.preventDefault();
					});
				});				
				$select.change(function()
				{
					...
					{
						var item = $(this);
						if (item.attr('index') == sel.selectedIndex) 
						{ 
			...
			// Set the new width
			var iSelectWidth = $select.outerWidth() + 8;
			var oSpan = $('span:first',$wrapper);
			var newWidth = iSelectWidth//(iSelectWidth > oSpan.innerWidth())?iSelectWidth+oLinkOpen.outerWidth():$wrapper.width();
...
(function($){
	$.fn.slides = function( option ) {
		...
		return this.each(function(){
			...
			var elem = $(this),
				...
				optional_text_box = control.children().find('.optional'),
				width = control.children().outerWidth(),
				height = control.children().outerHeight(),
				start = option.start - 1,
				effect = option.effect.indexOf(',') < 0 ? option.effect : option.effect.replace(' ', '').split(',')[0],
				paginationEffect = option.effect.indexOf(',') < 0 ? effect : option.effect.replace(' ', '').split(',')[1],
				next = 0, prev = 0, number = 0, current = 0, loaded, active, clicked, position, direction, imageParent, pauseTimeout, playInterval,play_temp=0,pause_temp=0;
					...
					// slide animation
						...
						if (option.autoHeight) {
							control.animate({
								...
								if(control.children(':eq('+ prev +')').hasClass('video')){
									var temp=control.children(':eq('+ prev +')').html();
									control.children(':eq('+ prev +')').html(temp);
								}
			...
			if (option.generatePlayStop) {
				$('.' + option.container, elem).after('<a href="#" class="playstop play">Play</a>');
				$('.' + option.container, elem).after('<a href="#" class="playstop stop">Stop</a>');
			}
			...
			// PLAy button
			$('.' + option.playbtn, elem).click(function(e){
				e.preventDefault();
				$('.' + option.playbtn).hide();
				$('.' + option.stopbtn).show();
				option.play=play_temp;
				option.pause=pause_temp;
				pause();
			});
			// STOP button
			$('.' + option.stopbtn, elem).click(function(e){
				e.preventDefault();
				$('.' + option.stopbtn).hide();
				$('.' + option.playbtn).show();
				option.play=0;
			...
			if (option.play) {
				...
				// store interval id
				elem.data('interval',playInterval);
				play_temp=option.play;
				pause_temp=option.pause;
			}else{
				$('.' + option.stopbtn).hide();
			}
			
			if (optional_text_box.length){
				optional_text_box.each(function(){
					var t=$(this),
						t_w=$(this).attr('data-width'),
						t_h=$(this).height();
						t_h_parent=$(this).parent().height()-$(this).parent().find('.caption').height()-30;
						t_w_parent=$(this).parent().width()-21;
					t.css('width',t_w);
					
					if (t.hasClass('pvertmiddle')){
						var temp=Math.floor((t_h_parent-t_h)/2);
						t.css({'top':temp});
					}
					if (t.hasClass('phorizmiddle')){
						var temp=Math.floor((t_w_parent-t_w)/2);
						t.css({'left':temp});
					}
				})
			}
	...
	$.fn.slides.option = {
		...
		generatePlayStop: true, // boolean, Auto generate next/prev buttons
		playbtn: 'play', // string, Class name for next button
		stopbtn: 'stop', // string, Class name for previous button
		pagination: true, // boolean, If you're not using pagination you can set to false, but don't have to
		generatePagination: true, // boolean, Auto generate pagination
		prependPagination: true, // boolean, prepend pagination
